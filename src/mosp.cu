 ...
 * where (a) is the GPU work of the K SOSP updates plus the SOSP on the
 * combined graph (the region the papers time) and (b) the wall time of
 * the whole run, from reading the inputs to writing the outputs.
 */
