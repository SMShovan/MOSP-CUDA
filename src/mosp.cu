 ...
 *                      (also enables NVTX ranges for nsys)
 ...
 *   RESULT gpu_compute_ms=<a> end_to_end_ms=<b>
 * where (a) is the GPU work of the K SOSP updates plus Steps 2-3 of the
 * MOSP update (the region the papers time) and (b) the wall time of the
 * whole run, from reading the inputs to writing the outputs.
 */
...
int main(int argc, char **argv) {
  ...
  // Load every kernel when the context is created instead of at its first
  // launch (lazy loading would land inside the first timed update). An
  // explicit CUDA_MODULE_LOADING in the environment still wins.
  setenv("CUDA_MODULE_LOADING", "EAGER", 0);
  setInstrumentation(!opt.timingCsv.empty());

  auto start = chrono::steady_clock::now();
  double contextMs = 0, loadMs = 0, canonicalizeMs = 0, writeMs = 0;
  {
    auto t = chrono::steady_clock::now();
    ScopedStage stage("cuda_context_init");
    cudaFree(nullptr);
    contextMs = msSince(t);
  }
  ...
  if (!opt.quiet) {
    ...
    printf("host   context %.1f ms, read inputs %.1f ms, canonicalize %.1f "
           "ms, apply batch %.1f ms, upload %.1f ms, download %.1f ms, "
           "write %.1f ms\n",
           contextMs, loadMs, canonicalizeMs, tm.applyBatch, tm.upload,
           tm.download, writeMs);
    for (int k = 0; k < K; ++k) {
      const SospStats &s = result.objectiveStats[k];
      printf("obj%d   SOSP update %.3f ms (invalidated %d, jump rounds %d, "
             "iterations %d, epochs %d, pushes %lld)\n",
             k, tm.objectives[k], s.invalidated, s.jumpRounds, s.iterations,
             s.epochs, s.pushes);
    }
  ...
  printf("RESULT gpu_compute_ms=%.3f end_to_end_ms=%.3f\n", tm.gpuCompute(),
         endToEnd);
  fflush(stdout);
