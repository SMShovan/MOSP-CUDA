...
bool parallelCombinedGraph(const string &originalCsrPrefix,
  ...
  treeStage.stop();
  ScopedStage mapStage("comb/1_membership_map_host");
  ...
  bool updated = parallelSOSPUpdate(tempCsrPrefix, blankDistPath, blankTreePath,
                                    insertPath, deletePath,
                                    /*objectiveIndex=*/0, source,
                                    distancesOutputPath, treeOutputPath);
